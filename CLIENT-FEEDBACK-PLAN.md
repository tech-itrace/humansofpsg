...
| 2 | Sync menu with main site, fix mobile bug | ⚠️ | [`Header.astro`](src/components/Header.astro) mirrors humansofpsg.org's menu tree, with a working mobile toggle + touch-friendly submenu carets, and desktop spacing/submenu styling now matched from the client's CSS/screenshot. Still needs a **mobile-specific QA pass** on a real device to confirm the originally reported mobile bug is actually fixed — it was never reproduced, only the general structure was verified. |
...
| 5 | Header/footer font size & position match main site (web + mobile) | ✅ | Matched main-menu item spacing (`padding: 0 10px`, `line-height: 30px`) from the client's CSS extract, and reworked the submenu dropdown to match the reference screenshot's generous spacing (larger padding, `min-width`, font-size). Main site's solid-black header bar / white logo box was deliberately **not** copied — client chose to keep our current light color scheme instead. |
...
| 4 | Editor content area adapts to web/mobile space | ⚠️ | No custom CMS layout code exists that could have broken this — it's Decap CMS's own default responsive behavior, which couldn't be visually verified in this environment (no browser available here). Needs a live QA pass on an actual device; revisit with a targeted fix if a specific problem is found. |
